    ...
    Route::group(['prefix' => 'dashboard'], function () {
        Route::get('/profile', function () {
            if (Auth::check()) {
                ...
                return view('dashboard.profile', ['title' => 'Digital Kas | Profile', 'data' => $userSession]);
            } else {
        ...
        Route::get('/anggota-kelas', function () {
            if (Auth::check()) {
                ...
                $userData = Kelas::select('id','kelas')->with('AnggotaKelas:id,nama_anggota,no_telepon,kelas_id')->find($userSession);

                return view('dashboard.anggota-kelas', ['title' => 'Digital Kas | Anggota Kelas' , 'data' => $userData]);
            } else {
                return redirect('/auth/login', 403)->with('error', 'Ups, kamu tidak memiliki akses.Silahkan login terlebih dahulu!');
            }
